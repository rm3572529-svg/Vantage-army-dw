...
(function () {
  ...
  // ── Wait for VantageDB to be ready ───────────
  function waitForVantageDB(callback, attempts) {
    attempts = attempts || 0;
    if (window.VantageDB && window.VantageDB.Auth) {
      callback();
    } else if (attempts < 20) {
      setTimeout(function() { waitForVantageDB(callback, attempts + 1); }, 100);
    } else {
      console.error("[Vantage] VantageDB failed to load after 2 seconds.");
      var c = document.getElementById("toast-container") || document.body;
      var t = document.createElement("div");
      t.style = "position:fixed;bottom:20px;left:50%;transform:translateX(-50%);background:#e53e3e;color:#fff;padding:12px 20px;border-radius:8px;z-index:9999;font-family:sans-serif;";
      t.textContent = "Firebase failed to load. Please refresh the page.";
      c.appendChild(t);
    }
  }
  ...
  document.addEventListener("DOMContentLoaded", function() {
    ...
    // Theme toggle
    document.querySelectorAll(".theme-toggle").forEach(function(btn) { btn.addEventListener("click", function() { Theme.toggle(); }); });
    ...
    // Page-specific init — wait for VantageDB first
    var page = document.body.dataset.page;

    waitForVantageDB(function() {
